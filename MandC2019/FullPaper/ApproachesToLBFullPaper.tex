...
To study the behavior of the original load balancing algorithm, the problem shown in Fig. \ref{partitioning_example} was run with a varying number of subsets and with a varying maximum triangle area, with the minimum allowable angle per triangle was kept constant at \ang{20}. Table \ref{og_table} shows the results of this parametric study using the original load balancing algorithm.
%\vspace{-1cm}
\begin{table}[htb]
\centering
...
\WHILE{$f > \text{tol}_{\text{subset}}$}
  \IF {$f_{D1} > \text{tol}_{\text{D1}}$}
    \STATE Redistribute the D1 cut planes.
  \ENDIF  
  
  \FOR {$d1$ in $D1$}
    \IF {$f_{D2,d1} > \text{tol}_{\text{D2}}$}
  ...
  \FOR{$d1$ in $D1$}
    \FOR{$d2$ in $D2$}
      \IF {$f_{D3,d2,d1} > \text{tol}_{\text{D3}}$ }
