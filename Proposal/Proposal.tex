...
\begin{equation}
\vec{\Omega}\cdot \vec\nabla \psi +\Sigma_t\psi  = S_{ext} + \frac{\chi}{4\pi}\int_{0}^{\infty}dE_i \nu\Sigma_f\phi + \int_{0}^{\infty}dE_i \int\int_{4\pi}d\Omega_i \Sigma_s(E_i\to E, \Omega_i\to\Omega)\psi,
\label{continuous transport}
\end{equation}
where $\vec{\Omega}_{m}\cdot \vec\nabla\psi$ is the leakage term and $\Sigma_t\psi$ is the collision term (absorption and outscatter). These are the loss terms of the neutron transport equation. The right hand side of Eq. \ref{continuous transport} represent the gain terms, where $S_{ext}$ is the external source of neutrons, $\frac{\chi}{4\pi}\int_{0}^{\infty}dE_i \nu\Sigma_f\phi$ is the fission term, and $\int_{0}^{\infty}dE_i \int\int_{4\pi}d\Omega_i \Sigma_s(E_i\to E, \Omega_i\to\Omega)\psi$ is the inscatter term, which represents all neutrons scattering in from energy $E_i$, as well as neutrons scattering in from energy $\Omega_i$.

Often, we like to solve the transport equation just for one group and one direction of neutrons. This results in the inscatter term dropping from the equation, as we've made the assumption that all neutrons will exist at one energy. As a result, we only care about neutrons that will no longer be at that energy. Thus our only neutron gain is from the external source and from fission. However, we also often wish to solve for several groups of neutrons, and Eq. \ref{multigroup} is used for this multigroup approach:
\begin{equation}
...
\end{equation}
where we have added the subscript $m$ and an extra summation of all directions $m^{\prime}$ scattering into direction $m$. Also shown is the superscript $l$, which introduces the concept of source iteration. Source iteration is a fundamental concept of the transport sweeps. The total source term is calculated, a sweep is performed, and is repeated until the $\psi^{(l+1)}-\psi^{(l)}$ converges to a specified tolerance.

Transport sweeps on unstructured meshes would not be possible without the work on the discontinuous finite element approximation performed by Reed and Willis at Los Alamos and the University of California\cite{Reed}. 
...
A transport sweep is set up by overlaying a domain with a finite element mesh. This divides the domain into a mesh of cells. The sweep then solves the transport equation cell by cell using a discontinuous finite element approach.
