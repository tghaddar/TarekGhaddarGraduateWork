...
\end{equation}
where $\vec{\Omega}\cdot \vec\nabla\psi$ is the leakage term and $\Sigma_t\psi$ is the total collision term (absorption and outscatter \tcr{so where's within group scattering?}). These are the loss terms of the neutron transport equation. The right hand side of Eq.~\eqref{continuous transport} represents the gain terms, where $S_{ext}$ is the external source of neutrons and $\int_{0}^{\infty}dE'\int_{4\pi}d\Omega'\Sigma_s(E'\to E, \Omega'\to\Omega)\psi(\vr,E',\vo')$ is the inscatter term, which represents all neutrons scattering from energy $E'$ and direction $\vo'$ into energy $dE$ about energy $E$ and $d\Omega$ about direction $\vo$.

Without loss of generality for the research problem at hand, we assume isotropic scattering for simplicity. The double differential scattering cross section, $\Sigma_s(E'\to E, \Omega'\to\Omega)$, no longer depends on direction, \tcr{should this comma stay?} and is divided by $4\pi$ to reflect isotropic behavior. This yields:
\begin{align}
\label{isotropic}
\vo \cdot \vec \nabla \psi(\vr,E,\vo) +\Sigma_t(\vr,E) \psi(\vr,E,\vo)  
...
\end{align}
where we \tcr{have} introduced the scalar flux as the integral of the angular flux:
\begin{equation}
...
As mentioned in the previous section, a transport sweep is set up by overlaying a domain with a finite element mesh. The sweep then solves the transport equation cell by cell using a discontinuous finite element approach. The order of which cell to solve first is given by a task dependence graph, as shown in Fig. \ref{tdg}. The transport sweep can be solved in parallel in order to obtain the solution faster, as well as distribute the memory to many processors for memory intensive cases. In PDT, a transport sweep can be performed on both a structured and unstructured mesh \tcr{so, if it is done, you don't have anything to propose in your proposal ... you should rewrite this a bit to keep the suspense} and the work proposed utilizes transport sweeps on an unstructured mesh.\tcr{what I am really looking for is a couple of sentence stating that parallel sweeps on unstructured meshes is a big deal, say why it is hard, i think it is hard for various reasons: first, because it is parallel and it should be done efficiently, second, because we use many many cores, not just 20, third, because of the nature of the sweep itself and the fact that we think we will need to keep non-concave sub-domain/ this last point is transport-specific. Then you should say that PDT already do massively // transport, but on structured grids. As part of previous efforts in PDT, researchers have come to outline 3 important properties for // sweeps ... and now you are back to your text }
...
\tcr{I would have expected to see the mc 2013 and mc 2015 PDT papers in the references...}
