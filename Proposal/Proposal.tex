...
Combining $n_x, n_y, \text{and } n_z$ with the cut line information in each direction allows us to compute the find grid units in each subset and along each boundary.  
...
\end{equation}
%end eq
where $N_u$ is the number of unknowns in the subset, $T_g$ is the grind time, $N_b$ is the number of unknowns along the subset boundaries, $T_\text{comm}$ is the communication time per unknown, and $M_l$ is the machine dependent latency multiplier. The edges in each graph are weighted according to this calculation.
...
\begin{itemize}
\item The PDT scheduler will not utilize any aggregation parameters in order to remain consistent with this time-to-solution estimator.
\end{itemize}
