...
Next, we discretize in angle using the discrete oridnate method (\text{find references}), whereby an angular quadrature $\left( \vo_m, w_m \right)_{1 \le m \le M}$ is used to solve the above equations along a given set of 
directions $\vo_m$:
\begin{equation}
