...
The KBA algorithm traditionally chooses $P_z = 1, A_m = 1, G = A_g = 1, A_x = N_x/P_x, A_y = N_y/P_y$, with $A_z$ being the selectable number of z-planes to be aggregated into each task. With $N_k = N_z/A_z$, each processor performs $N_{\text{tasks}} = 8MN_k$ tasks. In order to combat inefficiencies, KBA uses a "pipelining" or assembly line approach where new work is started before old work is fully completed. In other words, rather than a processor wait until all There are two variants to the KBA algorithm, "successive in angle successive in quadrant", and "simultaneous in angle, successive in quadrant".  KBA i

An angular quadrant consists of two opposing octants (also referred to as an octant-pair\cite{mpadams2013}) that have opposite sweep ordering. 
