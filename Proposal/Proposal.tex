...
The capability for PDT to generate and run on an unstructured mesh gained importance as the need to simulate irregular problems arose. However, there was a desire to keep logically Cartesian partitions in order to preserve the existing sweep mechanics in the code. The result was a grid that had logically Cartesian partitions (creating orthogonal ``subsets") with an unstructured mesh inside each subset. Subsets are obtained using cut planes in 3D and cut lines in 2D. Figure \ref{grid} demonstrates this functionality, with the first two subsets meshed using the Triangle Mesh Generator\cite{triangle}, a 2D mesh generator. 
