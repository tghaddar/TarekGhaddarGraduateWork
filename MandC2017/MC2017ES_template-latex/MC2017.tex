...
\tcr{remove 1 or 2 and add a sentence to the effect that more examples will be provided in the full paper}
...
Load imbalance reduction is observed throughout all three test cases for all three inputs, particularly the \tcr{first} two test cases, which were initially very unbalanced. However, there were many inputs run that had problems with $f > 1.1$, which means many problems were unbalanced by more than 10\% after application of the load balancing algorithm. 
