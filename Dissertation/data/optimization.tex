...
Thus the ``optimization suite'' should always have the cut types of previous work to test them.
...
At 20 seconds per run, this would take approximately 79 hours, which is completely unfeasible.
However, at 20 milliseconds per run, this would only take around 5 minutes, which is a reasonable amount of time to wait for balanced partitions.
