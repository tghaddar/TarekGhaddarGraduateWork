...
\end{itemize}
Using cut lines/cut planes to partition unstructured meshes may preserve \jcr{? and I would not put it in the present either. maybe may preserve but you need to explain the conditions under which something like this can be achieved.... this is missing} the provably optimal sweep partitioning \cite{mpadams2013,mpadams2015} of logically Cartesian grids if each partition has an equivalent number of cells.
...
\label{lbd_x_metric}
\end{equation}
Equation \ref{lbd_x_metric} defines the column-wise metric for layer $k$, or the maximum number of cells per column in layer $k$ divided by the average number of cells per column in layer $k$.\jcr{you should say what $N_{tot,k}$ is}
...
\jcr{why didn't you do the same with the level-2 mesh? this would have been great here, too.}
