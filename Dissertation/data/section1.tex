...
Obtaining the solution for current cells in the sweep is dependent on the solution of downwind cells.The base concept of solving the transport equation via the transport sweep is expanded on by four chapters in this dissertation.
...
This chapter also describes PDT's performance model, the precursor to the time-to-solution estimator described in Chapter \ref{cha:tts}.

\section{Unstructured Meshing and Load Balancing in PDT}

In order to solve a wider set of problems, and unstructured meshing capability was implemented in PDT. 
This chapter describes how unstructured meshes are described in PDT. Unstructured meshes may lead to imbalanced partitions, or different numbers of mesh cells per processor. 
To address this, two two load-balancing algorithms were implemented in PDT, the original load-balancing algorithm and the load-balancing-by-dimension algorithm, described in this chapter.
...
This chapter describes the time-to-solution estimator, a graph-based method that estimates the time it takes to sweep a problem given a mesh, partitioning scheme, and machine-specific parameters. 
