...
\end{block}
\centering
\includegraphics[scale = 0.21]{figures/UnstructureMesh.pdf}
\includegraphics[scale = 0.21]{figures/StructuredMesh.pdf}
\end{frame}

\begin{frame}[t]\frametitle{KBA}
\begin{block}{}
\begin{itemize}
\item KBA introduced a "pipelining" or assembly line approach where new work is started before old work is fully completed.
\end{itemize}
\end{block}
\centering
...
\begin{block}{}
\begin{itemize}
\item KBA primarily uses "successive in angle, successive in quadrant" approach.
...
\begin{block}{Cubit}
  \begin{itemize}
    \item Mesh is generated, load balanced and then read into PDT.
...
\section{Load Balancing and Motivation}
...
N_b &\approx(\text{num boundary cells})\cdot \text{unknowns per boundary cell}
\end{align}
\end{block}
\end{frame}
