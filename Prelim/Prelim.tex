...
\begin{itemize}
	\item Two (or more) paths arrive to a node at different time, when that node is free to solve, the path with the greater depth-of-graph remaining solves first.
\end{itemize}
\end{block}
\end{frame}
...
\begin{itemize}
	...
	\item Use time-to-solution estimator to optimize cut line locations with a Python optimization library (scipy.minimize, scipy.optimize, etc.)
