...
\begin{enumerate}
	...
	\begin{itemize}
		\item Solve and communication of each subset to it's neighbors.
...
\begin{frame}[t]{Longest Paths per Octant}
...
\includegraphics[trim={1cm 0cm 1cm 1cm},clip,scale=0.42]{figures/layer1partitioning.pdf}
\includegraphics[trim={1cm 0cm 1cm 1cm},clip,scale=0.42]{figures/layer2partitioning.pdf}
\end{frame}

\begin{frame}[t]\frametitle{Longest Path per Octant}
\begin{block}{After Mesh Information Only}
  \begin{enumerate}
  \item 0, 1, 3, 7
  \item 1, 3, 7, 6
  \item 2, 3, 1, 5
...
\begin{block}{}
\begin{itemize}
	\item Verify time-to-solution estimator matches PDT's sweep time.
	\item Test time-to-solution estimator against unstructured PDT meshes.
	\item Use time-to-solution estimator to optimize cut line locations via an optimization library.
	\item Explore adding the threads per subset as a variable to our parameter space in addition to the cut lines.
	\item Test optimizer against PDT's load balancing algorithms. 
\end{itemize}
\end{block}
\end{frame}
