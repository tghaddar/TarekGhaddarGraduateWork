...
A series of 162 inputs was constructed for each case \tcr{the reader may wonder why 162. do not let the reader ever wonder. say why 162, what is the span of parameters that you investigate, thus leading to 162 inputs, so it could be simple as rephrasing it as a parametric study as a function of the max triangle area for each test case}. The tabulated data in Appendix A show the the \tcr{the ???} parameters that will change in each input. The maximum triangle area varied from the coarsest possible to 0.01 cm\textsuperscript{2}, and the number of subsets, $N$, varies from 2x2 to 10x10 \tcr{wouldn't $\times$ be better here?}. 
...
\tcr{okay, that was a ton of results, now you need to draw conclusions from all of that before moving on. add them here.}
...
\tcr{this is confusing. why is the angular flux for mu>0 at x=0 equal to Psi inc /2 ? Why not just say it is Psi inc and it is the same value to all positive mu's? I think you are making it harder than it is. }
\begin{align}
...
\end{equation}
\tcr{ there could be different values of jinc due to the angular quadrature. can you add a table to show how jinc converges towards a unique value}
...
\tcr{in the defense, it would be good to show angular convergence as well}
